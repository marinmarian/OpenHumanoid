...
private:
    ...
    /// @brief baselink和运动中心
    Eigen::Matrix4d mat_baselink2motionlink_ = Eigen::Matrix4d::Identity();

    /// @brief imulink到baselink
    Eigen::Matrix4d mat_imulink2baselink_ = Eigen::Matrix4d::Identity();
                                             ...
                                             tf_listener_(std::make_shared<tf2_ros::TransformListener>(tf_buffer_))
{
    ...
    // Retry TF lookups — static publishers may not be ready yet
    for (int i = 0; i < 20; ++i) {
        ...
        RCLCPP_WARN(this->get_logger(), "Waiting for TF... retry %d/20", i + 1);
